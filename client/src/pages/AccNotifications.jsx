...
// AccNotifications.jsx
import React, { useState, useEffect } from 'react';
import Navbar from '../components/Navbar';
import { getBackendUrl } from '../utils/urlUtils'; // Import the utility function
 
// <Navbar /> // This line seems to be misplaced. It should be inside the component's return or a parent component.
 
const AccNotifications = () => {
  const [authStatus, setAuthStatus] = useState('idle');
  // Use the utility function to get the dynamic backend URL
    ...
    fetch(`${BACKEND_URL}/get-auth-url?state=${state}`)
      ...
      .then(data => {
        if (data.authUrl) {
          ...
          window.addEventListener("message", function(event) {
            // Verify origin for security
            // IMPORTANT: If your backend's REDIRECT_URI is hardcoded to localhost:8080,
            // but the browser's current origin is abc.localhost:5173,
            // event.origin might still be the popup's origin (localhost:8080).
            // You might need to adjust the origin check here or how you handle redirect_uri
            // based on how you registered your Autodesk app.
            const redirectOrigin = new URL(data.redirectUri).origin; //
            if (event.origin !== redirectOrigin) return;
            clearInterval(authCheckInterval);
            if (event.data.error) {
